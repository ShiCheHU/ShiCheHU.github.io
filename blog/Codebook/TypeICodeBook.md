...
无线通信系统中，无线信道的时间选择性、频率选择性、空间选择性、大尺度衰落和干扰都会导致传输性能下降。如何根据无线信道设计发送端和接收端处理，是 MIMO 系统的重要问题。LTE 和 NR 都采用 MIMO 技术提升频谱效率、覆盖和可靠性。为了匹配 MIMO 信道，发送端需要设计预编码、波束赋形、传输层数和调制编码方案，使发送信号尽量落在有利的空间方向和极化方向上。

在无线通信系统中，信道状态信息（Channel State Information，CSI）用于描述信道和链路质量。在 NR 中，CSI 可以包括 CSI-RS 资源指示（CRI）、SSB 资源指示（SSBRI）、秩指示（Rank Indicator，RI）、预编码矩阵指示（Precoding Matrix Indicator，PMI）、信道质量指示（Channel Quality Indicator，CQI）、参考信号接收功率（RSRP）和接收信号质量（RSRQ）等。网络侧根据 CSI 确定预编码矩阵、传输波束、rank 和 MCS。

在 CSI 中，PMI 用于指示推荐的预编码矩阵。码本是 NR 中用于有限比特表示预编码矩阵的协议化集合，它避免 UE 直接反馈完整复数预编码矩阵。为了介绍码本，首先需要明确预编码矩阵的物理含义，然后说明 NR 如何用码本索引描述预编码矩阵。

## 1.1 从 MIMO-OFDM 信道到预编码

在下行 MIMO-OFDM 系统中，第 $m$ 个 OFDM 符号、第 $k$ 个子载波上的频域接收信号可写为：
...
在接收端，也可以使用接收合并或均衡矩阵，例如 MRC、ZF、MMSE 或 IRC。它们和发送端预编码的目标类似，都是为了匹配等效信道、增强期望信号并抑制噪声或干扰。本文关注的是 NR 下行 Type I 码本，因此重点讨论发送端预编码矩阵 $\mathbf W[m,k]$ 的表示和反馈。

## 1.2 从预编码到码本

怎么获取预编码矩阵呢？首先，gNB 要获取或推断下行信道 $\mathbf H[m,k]$。然后，基于 $\mathbf H[m,k]$ 获取预编码矩阵 $\mathbf W[m,k]$。若 gNB 掌握完整下行 CSI，可以直接根据 SVD、MMSE、ZF、SLNR 或其他准则计算下行发送端预编码矩阵。

在 NR TDD 系统中，UE 侧配置探测参考信号（Sounding Reference Signal, SRS），用于 gNB 测量上行信道。经过射频校准后，gNB 可以利用上下行信道互易性推断下行空间方向，并确定下行发送预编码矩阵。但以下几类情况中，仅依赖互易性可能不够，需要下行 CSI 反馈或下行 CSI-RS 辅助。
1. 在 NR FDD 下行中，gNB 不能直接利用上行信道推断下行信道，因为上下行频率不同，信道不满足严格互易性。
2. 在NR TDD系统，UE侧接收天线数Y大于发送天线数X。这样，上行传输选择的X根天线为Y根天线中的一部分。要想获得所有接收天线的信道，需要轮询所有发送天线，需要Y/X次轮询。这在信道老化的情况下性能差。
3. 下行存在干扰。而基站无法通过互易性确定下行干扰。需要给予UE发送的SRS确定的信道，来确定一组预编码矩阵。然后，从这组预编码矩阵中选择干扰最小的预编码矩阵。这个时候，基站是通过发送预编码过的CSI-RS给UE，让UE做无PMI的CSI反馈，指示采用的预编码矩阵。

在获取下行CSI时，gNB通过CSI-RS获取的下行信道实际上要通过UE测量和计算，并且通知给gNB。而下行天线数多、带宽大，直接反馈$N_t \times L$ 个宽带预编码矩阵或者$N_t \times L \times N_{sb}$ 个子带预编码矩阵，每个元素采用Float 32 精度量化，所需要的比特数给上行传输带来的开销很大。而上行资源宝贵，因此反馈预编码矩阵不可行。

码本是有限个码字的集合，用于将预编码矩阵量化为有限个矩阵中的一个。如何设计码本是 NR 乃至 6G 都重点关注的方向。码本的设计需要考虑无线信道的空间、频率、极化和时间相关性，以实现更高的压缩率和表达精度。
码本设计主要考虑以下因素的影响：
1. 频率选择性：宽带和子带预编码；
2. 空间选择性：不同层之间复用一组二维DFT基向量；
3. 时间选择性：多普勒码本。

NR Type I和NR Type II都是考虑空间和频率选择性的码本。NR eType II进一步将频率相关性用基向量表达，压缩频域信息。其他包括多面板、多TRP码本等在Type I/II基础上考虑空间相关性进行压缩。到了Rel-18，提出了基于时间相关性的多普勒码本。

码本很多，Type I码本是基础，适用于单用户场景。所以，这篇文章主要介绍Type I码本的设计。

## 1.3 Type I 码本的定位

NR 下行码本反馈中，Type I 码本主要面向较低反馈开销的 beam-based CSI 反馈。支持单面板和多面板的Type I码本。Type I码本的每一次预编码向量通过一个二维的DFT向量以及关联的相位旋转表示。
它的设计目标不是完整恢复下行信道，而是让 UE 用少量比特报告一个适合下行传输的预编码方向。
...
Type I 码本的设计本质上是在性能和反馈开销之间折中。若候选波束越密、极化相位集合越大、支持 rank 越高，则码本越容易逼近理想预编码矩阵，但 PMI 比特数也会增加。
...
| $\Phi$ | 极化相位候选集合 | 增强两个极化端口的合成能力 |
| $L_{\max}$ | 最大支持 rank | 支持更高层数，但需要更多 RI/PMI 搜索 |
| 子带数量 $N_{\mathrm{SB}}$ | 频域反馈粒度 | 子带越细，频选适配越好，但反馈越大 |

若只考虑单层、单面板、双极化、同一空间波束加相位合成，则候选数可以粗略写为：

$$
\boxed{
N_{\mathrm{cb}}
=
N_1O_1\cdot N_2O_2\cdot |\Phi|
}
$$
...
$$
N_1=4,\quad N_2=2,\quad O_1=4,\quad O_2=4,\quad |\Phi|=4,
$$
...
$$
\lceil \log_2 N_{\mathrm{cb}}\rceil
$$

个比特。实际 NR 码本并不一定直接按这个粗略候选数编码 PMI，而是根据端口数、rank、码本模式、宽带/子带配置等规则拆分为不同 PMI 字段。但这个表达能说明基本权衡：

- 增大 $O_1,O_2$：空间波束更密，角度量化误差更小；
- 增大 $|\Phi|$：极化合成更精细，适合 XPR 较低或极化耦合明显的场景；
- 增加子带 PMI：能跟踪频率选择性，但反馈开销增加；
- 支持更高 rank：能提高空间复用能力，但码本搜索和反馈更复杂。

因此，Type I 码本不是追求无限精细的预编码表达，而是在有限反馈开销下选择足够好的结构化方向。
...
$$
N_1=4,\quad N_2=2,\quad N_t=2N_1N_2=16.
$$
...
\Phi=\left\{0,\frac{\pi}{2},\pi,\frac{3\pi}{2}\right\}.
$$
...
$$
\hat{\mathbf H}[k]\in\mathbb C^{N_r\times16},
\qquad
...
若 AoD 集中，即 $\Omega_{t,\ell}$ 分布很窄，则 $\mathbf A_t(\Omega_{t,\ell})$ 的主导方向接近某个 DFT 波束，Type I 码本容易选到接近最优的 $(l,m)$。若 AoD 分布很宽，则一个 Type I 单波束方向难以同时匹配所有路径，量化误差增大。
...
$$
\mathbf H[k]\mathbf w_{l,m,\phi}.
$$

这时增加极化相位候选集合 $|\Phi|$ 或使用更精细的子带相位，可能降低量化误差。反之，若 XPR 很高、交叉极化很弱，极化相位的收益可能小于空间波束方向选择的收益。

对 Kronecker 空间-极化相关模型：
...
如果 $\mathbf R_{t,\mathrm{space}}$ 的最大特征值明显大于其他特征值，Type I 的主导 DFT 波束量化通常有效；如果 $\mathbf R_{t,\mathrm{pol}}$ 的两个极化高度相关，则双极化端口提供的独立空间维度有限，高 rank 反馈不一定带来明显增益。
